...
# Polygon Erigon optimization goal

Main goal is to have fast and efficient polygon bor client.

**PRIMARY TARGET: Archive nodes** - All optimizations must preserve archive node functionality and data integrity.

## Agents

* create and prepare agents (coding, tests, git, docs etc)

## Main goal
* add trace_block functionality - check code in erigon (https://github.com/0xPolygon/erigon)
* add proper tests
* for each change, do a separate commit, do NOT include claude info
* for each feature, create git branch with reasonable name
* Each change must be commited into git, git commit message must contain optimization step idenfication and description
* Follow coding rules stated below

## Testing & Validation

### Reference RPC Endpoint
You may use the following Chainstack Polygon Erigon RPC endpoint to validate trace_block implementation and check correct values:
...
curl -X POST -H "Content-Type: application/json" \
  --data '{"method":"trace_block","params":["0x4cf4912"],"id":1,"jsonrpc":"2.0"}' \
  ${CHAINSTACK_RPC_URL}

# Example: Check specific transaction trace
# Block: 0x4cf4912
# TX: 0xc7054106020e48e65396b549e8b781d807d6598f6cdd486c9359f865976fe6bf
```

Use this endpoint to:
- Compare output format with Polygon Erigon
- Validate gas, gasUsed, and other field values
- Test edge cases and ensure 100% compatibility
...
* think twice before doint any deeper changes
* do NOT create complex code
* create valid and comprehensive documentation in fixes_docs folder
  * if it's fix, prepend FIX
  * if it's a feature, prepend FEATURE
  * append YYMMDD-VERSION to the file name as well
  * do NOT overwrite content of the file with another task
* git commit
  * do NOT append claude signature
  * briefly explain the commit
  * squash commits from one fix/feature into one
